...
Timing::Timing(const Config& config)
    ...
    int read_to_read_l = std::max(config.burst_cycle, config.tCCD_L);                   //burst_cycle 一次burut传输需要的时间？
    int read_to_read_s = std::max(config.burst_cycle, config.tCCD_S);
    int read_to_read_o = config.burst_cycle + config.tRTRS;
    int read_to_write = config.RL + config.burst_cycle - config.WL +                    //RL=CL+AL  WL=CWL+AL
                        config.tRTRS;
    ...
    // command READ                     这里的分类相当于是下一条命令与当前命令的关系
