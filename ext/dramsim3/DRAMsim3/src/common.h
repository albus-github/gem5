...
struct Command {
    Command() : cmd_type(CommandType::SIZE), hex_addr(0), IsPrefetch(false) {}
    Command(CommandType cmd_type, const Address& addr, uint64_t hex_addr, bool IsPrefetch)
        : cmd_type(cmd_type), addr(addr), hex_addr(hex_addr), IsPrefetch(IsPrefetch) {}
    // Command(const Command& cmd) {}

    bool IsValid() const { return cmd_type != CommandType::SIZE; }                  //SIZE类型判断为无效，其余命令类型判断相应的类型的布尔值
