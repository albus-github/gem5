...
Address Config::AddressMapping(uint64_t hex_addr) const {                   
    hex_addr >>= shift_bits;                                                                                    //通过右移截断代表一次请求访问的字节数的标志位，剩下实际的地址位
    int channel = (hex_addr >> ch_pos) & ch_mask;                                     //通过相应位的掩码来提取相应的数值
    ...
    std::cout<<"hex_addr: "<<hex_addr<<", "<< std::bitset<32> (hex_addr)<<std::endl;
    std::cout<<"The position of the address in the hex_addr is: "<<"channel: "<<ch_pos<<", "<<"rank: "<<ra_pos<<", "<<"bg: "<<bg_pos<<", "<<"bank: "<<ba_pos<<", "<<"row: "<<ro_pos<<", "<<"column: "<<co_pos<<std::endl;
    std::cout<<"channel: "<<channel<<", "<<"rank: "<<rank<<", "<<"bg: "<<bg<<", "<<"ba: "<<ba<<", "<<"ro: "<<ro<<", "<<"co: "<<co<<std::endl;*/
...
void Config::InitSystemParams() {                   //读取configs中ini文件的配置，其中相应的参数应该可以在ini文件中修改
    const auto& reader = *reader_;
    ...
    address_mapping = reader.Get("system", "address_mapping", "chrobabgraco");                  //决定地址映射的方式
...
void Config::SetAddressMapping() {
    ...
    request_size_bytes = bus_width / 8 * BL;                                                                                       //一次请求能访问的字节数
    shift_bits = LogBase2(request_size_bytes);                                                                                  //地址中用来标注一次请求访问的字节数的位？
    ...
    std::map<std::string, int> field_widths;                                                                                          //通过取对数确定宽度
    ...
    std::vector<std::string> fields;                                                                                 //每两个字符提取为一个token放入fields
    for (size_t i = 0; i < address_mapping.size(); i += 2) {
        std::string token = address_mapping.substr(i, 2);
        fields.push_back(token);
    }

    std::map<std::string, int> field_pos;                                                                   //存储每个字段的位置
    int pos = 0;
    while (!fields.empty()) {
    ...
    ch_mask = (1 << field_widths.at("ch")) - 1;                                                         //将与相应位宽的低位置1，通过“与”操作选取相应位置
