...
Command CommandQueue::FinishRefresh() {         //实际上就是发送刷新队列中下一条刷新命令
...
bool CommandQueue::ArbitratePrecharge(const CMDIterator& cmd_it,                    //判断是否要precharge，考虑行缓冲命名和命中限制（为什么要限制行缓冲命中的次数？？？）
                                      const CMDQueue& queue) const {
    auto cmd = *cmd_it;

    for (auto prev_itr = queue.begin(); prev_itr != cmd_it; prev_itr++) {                       //通过判断是否为行缓冲命中来决定是否要precharge
    ...
    for (auto pending_itr = cmd_it; pending_itr != queue.end(); pending_itr++) {                    //检查未完成的命令中是否有行缓冲命中，如果有则将pending_row_hits_exist置为true
...
Command CommandQueue::GetFirstReadyInQueue(CMDQueue& queue) const {
    for (auto cmd_it = queue.begin(); cmd_it != queue.end(); cmd_it++) {
        ...
        if (cmd.cmd_type == CommandType::PRECHARGE) {
            if (!ArbitratePrecharge(cmd_it, queue)) {                                 //实现fr-fcfs，当行冲突要precharge时，如果命令队列中有行缓冲命中的命令，则跳过precharge
                continue;
            }
        } else if (cmd.IsWrite()) {
